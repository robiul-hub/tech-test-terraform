...
resource "aws_db_instance" "my-test-sql" {
  ...
  vpc_security_group_ids  = [aws_security_group.DB-sg.id] //aws_security_group.my-rds-sg.id //["${aws_security_group.my-rds-sg.id}"]
}

resource "aws_db_subnet_group" "my-rds-db-subnet" {
  name       = "my-rds-db-subnet"
  subnet_ids = [var.rds_subnet1, var.rds_subnet2] //["${var.rds_subnet1}", "${var.rds_subnet2}"]
...
resource "aws_security_group_rule" "my-rds-sg-rule" {
  ...
  to_port           = 3306
  type              = "ingress"
  source_security_group_id   = var.security_groups //["${var.security_groups}"]
}
...
resource "aws_security_group" "DB-sg" {
  ...
  vpc_id = var.vpc_id //vpc_id      = aws_vpc.main.id
  ...
  ingress {
    ...
    to_port     = 3306
    protocol    = "tcp"
    security_groups   = [var.security_groups] //cidr_blocks = ["0.0.0.0/0"]
  }
